      ...
      it "verifies that a license exception has standard spdx ids" do
        formula_text = <<~RUBY
          class Cask < Formula
            url "https://github.com/cask/cask/archive/v0.8.4.tar.gz"
            head "https://github.com/cask/cask.git"
            license "GPL-3.0-or-later" => { with: "#{custom_spdx_exception}" }
          end
        RUBY
        ...
        fa.audit_license
        expect(fa.problems.first).to match <<~EOS
          Formula cask contains invalid or deprecated SPDX license exceptions: ["#{custom_spdx_exception}"].
